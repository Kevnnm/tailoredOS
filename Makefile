...
$(kernel_objects) : build/kernel/%.o : src/impl/kernel/%.c
	mkdir -p $(dir $@) && \
    printf "\e33;1mCompiling c files\e[0m $@\n" && \
	x86_64-elf-gcc -std=c11 -c -I src/intf -ffreestanding $(patsubst build/kernel/%.o, src/impl/kernel/%.c, $@) -o $@
...
$(x86_64_asm_objects) : build/x86_64/%.o : src/impl/x86_64/%.asm
	mkdir -p $(dir $@) && \
    printf "\e[33;1mCompiling assembly files\e[0m $@\n" && \
